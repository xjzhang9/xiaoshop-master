...
import com.xjzhang.base.utils.JsonConvertUtil;
...
public class JwtFilter extends BasicHttpAuthenticationFilter  {
    ...
    private boolean refreshToken(ServletRequest request, ServletResponse response) {
        ...
        if (redisTemplate.hasKey(RedisConstant.PREFIX_SHIRO_REFRESH_TOKEN + account)) {
            // Redis中RefreshToken还存在，获取RefreshToken的时间戳
            String currentTimeMillisRedis = redisTemplate.opsForValue().get(RedisConstant.PREFIX_SHIRO_REFRESH_TOKEN + account).toString();
            // 获取当前AccessToken中的时间戳，与RefreshToken的时间戳对比，如果当前时间戳一致，进行AccessToken刷新
            if (JwtUtil.getClaim(token, "timeStamp").equals(currentTimeMillisRedis)) {
                // 获取当前最新时间戳
                ...
                redisTemplate.opsForValue().set(RedisConstant.PREFIX_SHIRO_REFRESH_TOKEN + account, currentTimeMillis,
    ...
    private void response401(ServletRequest req, ServletResponse resp, String msg) {
        ...
        try {
            ...
            String data = JsonConvertUtil.objectToJson(ResWrapper.error(ErrorCodeEnum.AUTH10010001.code(), msg));
            out.append(data);
        } catch (IOException e) {
